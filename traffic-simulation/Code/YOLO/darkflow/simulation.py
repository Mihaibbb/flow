...
LAT = 44.43571151877619
LONG = 26.094031315614625
URL = 'http://server.mihaibarbu.com/send-traffic'
...
def printStatus():
    ...
    for i in range(0, noOfSignals):
        if(i==currentGreen):
            if(currentYellow==0):
                arr.update({f"traffic{i}":"GREEN"})
                #print(" GREEN TS",i+1,"-> r:",signals[i].red," y:",signals[i].yellow," g:",signals[i].green)
            else:
                arr.update({f"traffic{i}":"YELLOW"})
                #print("YELLOW TS",i+1,"-> r:",signals[i].red," y:",signals[i].yellow," g:",signals[i].green)
        else:
            arr.update({f"traffic{i}":"RED"})
            #print("   RED TS",i+1,"-> r:",signals[i].red," y:",signals[i].yellow," g:",signals[i].green)
    r = requests.post(URL, json = arr, verify=False)
...
class Main:
    ...
    font = pygame.font.Font("rough_anthem/rough.ttf", 20)
